@section('meta')
    <title>{{ __('Article sections') }} &raquo; {{ $section->name }} &raquo; {{ __('Delete') }} - {{ __('Backend') }} | {{ config('app.name') }}</title>
@endsection
    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('article-sections.show', $section) }}">{{ $section->name }}</a></li>
                <li class="breadcrumb-item active" aria-current="page">{{ __('Delete') }}</li>
            </ol>
        </nav>
        <div class="btn-toolbar mb-3">
            <a class="btn btn-outline-dark" href="{{ route('article-sections.show', $section) }}">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <p class="card-text">{{ __('Removing this article section is permanent and cannot be undone. Are you sure?') }}</p>
            </div>
