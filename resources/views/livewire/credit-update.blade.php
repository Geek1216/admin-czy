@section('meta')
    <title>{{ __('Credits') }} &raquo; {{ $credit->title }} &raquo; {{ __('Edit') }} - {{ __('Backend') }} | {{ config('app.name') }}</title>
@endsection
    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('credits.show', $credit) }}">{{ $credit->title }}</a></li>
                <li class="breadcrumb-item active" aria-current="page">{{ __('Edit') }}</li>
            </ol>
        </nav>
        <div class="btn-toolbar mb-3">
            <a class="btn btn-outline-dark" href="{{ route('credits.show', $credit) }}">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <p class="card-text">{{ __('Update existing credit information here.') }}</p>
            </div>
                    ...
                    <div class="col-md-12 col-lg-8">
                        <form class="mb-0" wire:submit.prevent="update">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-8">
                                    <input class="form-control @error('order') is-invalid @enderror" id="credit-order" required type="number" wire:model="order">
                                    @error('order')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
