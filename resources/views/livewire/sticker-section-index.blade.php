@section('meta')
    <title>{{ __('Sticker sections') }} - {{ __('Backend') }} | {{ config('app.name') }}</title>
@endsection
    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ __('Sticker sections') }}</li>
            </ol>
        </nav>
        <div class="btn-toolbar mb-3">
            <a class="btn btn-success ml-auto" href="{{ route('sticker-sections.create') }}">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <h5 class="card-title text-primary">{{ __('Sticker sections') }}</h5>
                <p class="card-text">
                    {{ __('List and manage sticker sections here.') }}
                ...
                <table class="table mb-0">
                    ...
                    @forelse ($sections as $section)
                        <tr>
                            ...
                            <td>{{ $section->name }}</td>
                            <td>{{ $section->order }}</td>
                            <td>{{ $section->stickers()->count() }}</td>
                            <td>{{ $section->created_at->format('d/m/Y H:i:s') }}</td>
                            <td>
                                <a class="btn btn-outline-dark btn-sm" href="{{ route('sticker-sections.show', $section) }}">
                                    <i class="fas fa-eye mr-1"></i> {{ __('Details') }}
                                </a>
                                <a class="btn btn-info btn-sm" href="{{ route('sticker-sections.update', $section) }}">
                                    <i class="fas fa-feather mr-1"></i> {{ __('Edit') }}
                                </a>
                                @can('administer')
                                    <a class="btn btn-danger btn-sm" href="{{ route('sticker-sections.destroy', $section) }}">
                    ...
                    @empty
                        <tr>
                            <td class="text-center text-muted" colspan="6">{{ __('Could not find any sticker sections to show.') }}</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
            ...
            @endif
            <div class="card-body border-top">
                {{ __('Showing :from to :to of :total sticker sections.', ['from' => $sections->firstItem() ?: 0, 'to' => $sections->lastItem() ?: 0, 'total' => $sections->total()]) }}
            </div>
        </div>
    </div>
</div>
