@section('meta')
    <title>{{ __('Comments') }} &raquo; {{ $comment->comment_short }} &raquo; {{ __('Delete') }} - {{ __('Backend') }} | {{ config('app.name') }}</title>
@endsection
    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('comments.show', $comment) }}">{{ $comment->comment_short }}</a></li>
                <li class="breadcrumb-item active" aria-current="page">{{ __('Delete') }}</li>
            </ol>
        </nav>
        <div class="btn-toolbar mb-3">
            <a class="btn btn-outline-dark" href="{{ route('comments.show', $comment) }}">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <p class="card-text">{{ __('Removing this comment is permanent and cannot be undone. Are you sure?') }}</p>
            </div>
