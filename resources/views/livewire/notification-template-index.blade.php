@section('meta')
    <title>{{ __('Notification templates') }} - {{ __('Backend') }} | {{ config('app.name') }}</title>
@endsection
...
<div>
    <div class="container">
        ...
        <div class="btn-toolbar mb-3">
            <a class="btn btn-success ml-auto" href="{{ route('notification-templates.create') }}">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <h5 class="card-title text-primary">{{ __('Notification templates') }}</h5>
                <p class="card-text">
                    {{ __('List and manage notification templates here.') }}
                    ...
                    <div class="row">
                        <div class="col-sm-6 col-md-4 col-lg-3">
                            <div class="form-group mb-sm-0">
                                ...
                                <input id="filter-search" class="form-control" placeholder="{{ __('Enter title or body') }}&hellip;" wire:model.debounce.500ms="search">
                            </div>
                        </div>
            ...
            <div class="table-responsive">
                <table class="table mb-0">
                    ...
                    @forelse ($templates as $template)
                        <tr>
                            <td>{{ $template->id }}</td>
                            <td>{{ $template->title_short }}</td>
                            <td>{{ $template->schedules()->count() }}</td>
                            <td>{{ $template->created_at->format('d/m/Y H:i:s') }}</td>
                            <td>
                                <a class="btn btn-outline-dark btn-sm" href="{{ route('notification-templates.show', $template) }}">
                                    <i class="fas fa-eye mr-1"></i> {{ __('Details') }}
                                </a>
                                <a class="btn btn-info btn-sm" href="{{ route('notification-templates.update', $template) }}">
                                    <i class="fas fa-feather mr-1"></i> {{ __('Edit') }}
                                </a>
                                @can('administer')
                                    <a class="btn btn-danger btn-sm" href="{{ route('notification-templates.destroy', $template) }}">
                    ...
                    @empty
                        <tr>
                            <td class="text-center text-muted" colspan="5">{{ __('Could not find any notification templates to show.') }}</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
            ...
            @endif
            <div class="card-body border-top">
                {{ __('Showing :from to :to of :total notification templates.', ['from' => $templates->firstItem() ?: 0, 'to' => $templates->lastItem() ?: 0, 'total' => $templates->total()]) }}
            </div>
        </div>
    </div>
</div>
