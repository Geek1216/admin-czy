@section('meta')
    <title>{{ __('Items') }} - {{ __('Backend') }} | {{ config('app.name') }}</title>
@endsection
...
<div>
    <div class="container">
        ...
        <div class="btn-toolbar mb-3">
            <a class="btn btn-success ml-auto" href="{{ route('items.create') }}">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <h5 class="card-title text-primary">{{ __('Items') }}</h5>
                <p class="card-text">
                    {{ __('List and manage gifts/items here.') }}
                ...
                <table class="table mb-0">
                    ...
                    @forelse ($items as $item)
                        <tr>
                            <td>{{ $item->id }}</td>
                            <td>
                                <div class="text-hide img-screenshot rounded" style="background-image: url('{{ Storage::disk(setting('filesystems_cloud', config('filesystems.cloud')))->url($item->image) }}')">&nbsp;</div>
                            </td>
                            <td>{{ $item->name }}</td>
                            <td>{{ $item->price }} <span class="text-muted">{{ __('Credits') }}</span></td>
                            <td>{{ $item->created_at->format('d/m/Y H:i:s') }}</td>
                            <td>
                                <a class="btn btn-outline-dark btn-sm" href="{{ route('items.show', $item) }}">
                                    <i class="fas fa-eye mr-1"></i> {{ __('Details') }}
                                </a>
                                <a class="btn btn-info btn-sm" href="{{ route('items.update', $item) }}">
                                    <i class="fas fa-feather mr-1"></i> {{ __('Edit') }}
                                </a>
                                @can('administer')
                                    <a class="btn btn-danger btn-sm" href="{{ route('items.destroy', $item) }}">
                    ...
                    @empty
                        <tr>
                            <td class="text-center text-muted" colspan="6">{{ __('Could not find any items to show.') }}</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
            ...
            @endif
            <div class="card-body border-top">
                {{ __('Showing :from to :to of :total items.', ['from' => $items->firstItem() ?: 0, 'to' => $items->lastItem() ?: 0, 'total' => $items->total()]) }}
            </div>
        </div>
    </div>
</div>
