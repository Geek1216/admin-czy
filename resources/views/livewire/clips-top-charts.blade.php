<div>
    ...
    <div class="row">
        @foreach (['viewed', 'liked', 'saved', 'commented'] as $type)
            <div class="col-sm-6 col-md-4 col-lg-3">
                <div class="card border-0 shadow-sm mb-3">
                    <div class="card-body p-3">
                        @if ($results[$type])
                            <a class="float-right text-decoration-none stretched-link" href="{{ route('clips.show', $results[$type]) }}">
                        ...
                        <p class="card-text">
                            @if ($results[$type])
                                {{ __('#:id by @:username', ['id' => $results[$type]->id, 'username' => $results[$type]->user->username]) }}
                            @else
                                {{ __('n/a') }}
                            @endif
                        </p>
