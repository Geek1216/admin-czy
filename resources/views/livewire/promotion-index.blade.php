@section('meta')
    <title>{{ __('Promotions') }} - {{ __('Backend') }} | {{ config('app.name') }}</title>
@endsection
...
<div>
    <div class="container">
        ...
        <div class="btn-toolbar mb-3">
            <a class="btn btn-success ml-auto" href="{{ route('promotions.create') }}">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <h5 class="card-title text-primary">{{ __('Promotions') }}</h5>
                <p class="card-text">
                    {{ __('List and manage promotional banners here.') }}
                    ...
                    <div class="row">
                        <div class="col-sm-6 col-md-4 col-lg-3">
                            <div class="form-group mb-sm-0">
                                ...
                                <input id="filter-search" class="form-control" placeholder="{{ __('Enter title or description') }}&hellip;" wire:model.debounce.500ms="search">
                            </div>
                        </div>
                        <div class="col-sm-6 col-md-4 col-lg-3">
                            <div class="form-group mb-md-0">
                                <label for="filter-sticky">{{ __('Sticky?') }}</label>
                                <select id="filter-sticky" class="form-control" wire:model="sticky">
                ...
                <table class="table mb-0">
                    ...
                    @forelse ($promotions as $promotion)
                        <tr>
                            <td>{{ $promotion->id }}</td>
                            <td>
                                <div class="text-hide img-screenshot rounded" style="background-image: url('{{ Storage::disk(setting('filesystems_cloud', config('filesystems.cloud')))->url($promotion->image) }}')">&nbsp;</div>
                            </td>
                            <td>{{ $promotion->title_short }}</td>
                            <td>
                                @if ($promotion->sticky)
                            ...
                            <td>
                                <a class="btn btn-outline-dark btn-sm" href="{{ route('promotions.show', $promotion) }}">
                                    <i class="fas fa-eye mr-1"></i> {{ __('Details') }}
                                </a>
                                <a class="btn btn-info btn-sm" href="{{ route('promotions.update', $promotion) }}">
                                    <i class="fas fa-feather mr-1"></i> {{ __('Edit') }}
                                </a>
                                @can('administer')
                                    <a class="btn btn-danger btn-sm" href="{{ route('promotions.destroy', $promotion) }}">
                    ...
                    @empty
                        <tr>
                            <td class="text-center text-muted" colspan="6">{{ __('Could not find any promotions to show.') }}</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
            ...
            @endif
            <div class="card-body border-top">
                {{ __('Showing :from to :to of :total promotions.', ['from' => $promotions->firstItem() ?: 0, 'to' => $promotions->lastItem() ?: 0, 'total' => $promotions->total()]) }}
            </div>
        </div>
    </div>
</div>
