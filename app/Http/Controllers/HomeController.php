...
class HomeController extends Controller
{
    public function index()
    {
        ...
        for ($i = 23; $i >= 0; $i--) {
            if ($i === 0) {
                $fromStory = now()->startOfHour();
                $untilStory = now()->endOfHour();
            } else {
                $fromStory = now()->startOfHour()->subHours($i);
                $untilStory = now()->endOfHour()->subHours($i);
            }
            $countStory = views(Story::class)
                ->period(Period::create($from, $until))
                ->count();
            $label = sprintf(
                '%s - %s',
        ...
        $devices = DB::table('devices')
            ...
            ->first();
        return view('home', compact('views','devices', 'viewsStory'));
    }
}
